/**
 * Cell class for the Doping variant. 
...
public class Cell2 {
    ...
    private ArrayList<Igel2> contents;
    public boolean trap;
     ...
     */
    public Cell2() {
        this.contents=new ArrayList<Igel2>();
    }
    ...
    public void createTrap(int r, int c) {
        for (int t=0; t<Board2.TRAPS.length; t++) {
            if ((t==r) && (Board2.TRAPS[t]==c)) {
                Igel2 trapTok=new Igel2('@');
     ...
     */
    public void addToken(Igel2 token) {
        contents.add(token);
    }
     ...
     */
    public Igel2 popToken() {
        if (isEmpty()) {
            return null;
        }
        else {
            Igel2 popped=contents.get(contents.size()-1);
     ...
     * @return
     */
    public int countTokens(Player2 player) {
        int count=0;
        for (int i=0; i<contents.size(); i++) {
            Igel2 piece=contents.get(i);
     ...
     * @return
     */
    public boolean contains(Player2 player) {
        for (int i=0; i<contents.size(); i++) {
            Igel2 piece=contents.get(i);
