...
public class Cell {
    ...
    private ArrayList<Igel> contents;
    public boolean trap;
     ...
     */
    public Cell() {
        this.contents=new ArrayList<Igel>();
    }
    ...
    public void createTrap(int r, int c) {
        for (int t=0; t<Board.TRAPS.length; t++) {
            if ((t==r) && (Board.TRAPS[t]==c)) {
                Igel trapTok=new Igel('@');
     ...
     */
    public void addToken(Igel token) {
        contents.add(token);
    }
     ...
     */
    public Igel popToken() {
        if (isEmpty()) {
            return null;
        }
        else {
            Igel popped=contents.get(contents.size()-1);
