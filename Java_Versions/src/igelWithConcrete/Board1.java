/**
 * Board Class for the Pillar variant of Igel Aergern. In this version
 * canMoveSideways and canMoveForward behave and percieve traps differently than in the base game.
 * There is also no longer a way to manipulate or toggle traps.
 * 
...
public class Board1 {
     ...
     */
    public Board1 () {
        this.board=new Cell1[ROWS][COLUMNS];
    ...
    public void placeToken(int row, int col, char color) {
        Cell1 cell=board[row][col];
        Igel1 token=new Igel1(color);
        cell.addToken(token);
        board[row][col]=cell;
    }
     ...
     * @return
     */
    public boolean canMoveSideways(int row, int col, Player1 player) {
        if (row==0 && board[row+1][col].trap){
            return false;
        }
        else if (row==5 && board[(row-1)][col].trap){
            return false;
        }
        else if (row>0 && row<5 && (board[(row-1)][col].trap) && (!(board[(row+1)][col].trap))) {
    ...
    public void moveSideways(int row, int col, int dir) {
        if (dir==-1){
            Igel1 popped=board[row][col].popToken();
    ...
    public void moveForward(int row, int col) {
        Igel1 player=board[row][col].popToken();
        col++;
     ...
     */
    public boolean isWinner(Player1 player) {
     ...
     * @return
     */
    public boolean isPlayerAt(int row, int col, Player1 player) {
     ...
     * @return
     */
    public boolean isPlayerTop(int row, int col, Player1 player) {
        if (board[row][col].isEmpty()){
            return false;
        }
        else{
    ...
    public boolean emptyRow(int row){
        for (int c=0; c<(COLUMNS-1); c++) {
            if (!board[row][c].isEmpty() && !board[row][c+1].isTrap()){
                return false;
            }                                                            
     ...
     * @return
     */
    private int countColumn(Player1 player){
        int counter=0;
        for (int r=0; r<ROWS; r++){
