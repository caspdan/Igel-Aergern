 ...
 * This version implements the pillar variant of 
 * the Igel Aergern game and renders traps un-occupiable.
 * 
 * Changes were made in lines 120-128, and 151-156.
 * 
...
public class IgelAergern1 {

    private Board1 board;
    private IgelView1 view;
    ...
    private void createPlayers() {
        ...
        for (int i = 0; i < numPlayers; i++) {
            players.add(new Player1(Player1.COLORS[i]));
        }
    }
            ...
            for (int c=0; c<Board1.COLUMNS; c++){
                if (board.stackHeight(r, c)!=0 && (board.canMoveSideways(r, c, players.get(currentPlayer)))) {
                    ...
                    list=new ArrayList<Integer>();
                        ...
                        list.add((r));
                        list.add((c));
                        if (r>0 && r<5 && (board.isTrap((r+1),c))){
                            list.add(-1);
                        }
                        else if (r>0 && r<5 && (board.isTrap((r-1),c))){
                            list.add(1);
                        }
                        else{
                            list.add(0);
    ...
    private void goForward(int currentPlayer, int die){
        ...
        if (board.emptyRow(die)){
            ...
            int allowed=0;
            for (int c=0; c<(Board1.COLUMNS-1); c++){
            ...
            if(allowed==0){
                ...
                while (m<1){
                    ...
                    boolean isOk=isInRange(col, 1, (Board1.COLUMNS-1));
    ...
    private int getForChoice(int die, int allowed){
        ...
        if (answer.length()==1){
            if (!isDigit(answer)){
                return (Board1.COLUMNS+1);
            }
            else if (Integer.decode(answer)>Board1.COLUMNS) {
                return (Board1.COLUMNS+1);
            }
